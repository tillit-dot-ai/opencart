...
class ModelExtensionPaymentTillit extends Model {
	public function getMethod($address, $total) {
		...
		if ($status) {
			$method_data = array(
				'code'       => 'tillit',
				'title'      => sprintf($this->config->get('payment_tillit_title'), $this->config->get('payment_tillit_invoice_days')),
				'terms'      => $this->config->get('payment_tillit_sub_title'),
    ...
    {
        $result = $this->getTillitOrderPaymentData($order_id);
        if ($result) {
        	...
        	$query = $this->db->query("INSERT INTO " . DB_PREFIX . "tillit_order SET order_id = '" . (int)$order_id . "', id = '" . $this->db->escape($payment_data['id']) . "', merchant_reference = '" . $this->db->escape($payment_data['merchant_reference']) . "', state = '" . $this->db->escape($payment_data['state']) . "', status = '" . $this->db->escape($payment_data['status']) . "', day_on_invoice = '" . $this->db->escape($payment_data['day_on_invoice']) . "', invoice_url = '" . $this->db->escape($payment_data['invoice_url']) . "'");
        }
    }
